...
export function PersonalityAnalysisChat({ onStageChange, currentStage }: PersonalityAnalysisChatProps) {
  ...
  useEffect(() => {
    const initMessage: Message = {
      id: '1',
      content: language === 'zh' 
        ? `你好！我是你的AI人格分析师。我会通过一系列问题来深入了解你的性格特征、价值观和偏好，从而为你提供更精准的匹配建议。

这个过程大约需要10-15分钟，你可以选择用文字回答或者点击麦克风图标进行语音交流。

让我们开始吧！首先请告诉我，你目前${user?.metadata?.request_type === '找对象' ? '在寻找romantic伙伴' : '在寻找合作伙伴'}时，最看重对方的哪些特质？`
        : `Hello! I'm your AI personality analyst. I'll understand your personality traits, values, and preferences through a series of questions to provide you with more accurate matching suggestions.
...
Let's begin! First, please tell me what qualities do you value most when ${user?.metadata?.request_type === '找对象' ? 'looking for a romantic partner' : 'looking for a collaboration partner'}?`,
      sender: 'ai',
      timestamp: new Date(),
      type: 'text'
    }
  ...
  const handleSendMessage = async (messageText?: string) => {
    ...
    try {
      ...
      const systemPrompt = language === 'zh' 
        ? `你是一个专业的人格分析师，专门通过对话来了解用户的性格特征。你的目标是：
...
用户类型：${user?.metadata?.request_type || '找队友'}

请根据用户的回答继续对话，深入了解他们的性格。`
        : `You are a professional personality analyst who understands users' personality traits through conversation. Your goals are:
...
2. Adjust question depth based on conversation rounds:
   ...
   - 4-8 rounds: Deep exploration of personality traits and values
   - 9-12 rounds: Specific scenario questions and preference details
...
3. Question style:
   ...
   - Use scenario assumptions to understand user reactions
...
User type: ${user?.metadata?.request_type || 'finding teammates'}

Please continue the conversation based on the user's response, diving deeper into their personality.`
