...
# 由于5000端口被系统占用，自动切换到备用端口
if lsof -ti :5000 >/dev/null 2>&1; then
    echo "⚠️  端口5000被系统进程占用，自动切换到8001端口"
    API_PORT=8001
fi

# 检查服务是否已运行
check_service_running $API_PORT
        ...
        for pid in $pids; do
            ...
            if [ ! -z "$process_info" ]; then
                ...
                if echo "$process_info" | grep -q "python\|uvicorn\|main_api"; then
                    ...
                    echo "   💡 或者使用不同的端口: API_PORT=8001 $0"
                    exit 1
                fi
            fi
        done
...
echo ""
echo "💡 使用不同端口: API_PORT=8001 $0"
echo "按 Ctrl+C 停止服务"
echo ""

# 使用统一的启动方式
python backend/main.py simple --simple-port ${API_PORT} 